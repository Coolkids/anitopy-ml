...
- [x] V11 已生成 299,992 条唯一样本，并按作品隔离、模板条目全覆盖划分为训练 251,096、验证 21,307、新冻结测试 27,589 条；42 个模板条目均覆盖到三个分区。首个权重初始化训练的验证实体 F1 为 98.83%、名称与别名联合为 99.11%；模板清理后重建下一版本，不再对当前数据复测其余种子。
...
| 13 | 模型与训练器 | 已完成 | V10 三种子 CUDA 训练完成；V11 增加权重初始化和批量推理，首个续训验证实体 F1 为 98.83%、训练和验证对齐问题均为零。详见 `reports/V11首个权重初始化验证复核.md`。 |
...
| 15 | 正式训练与对照实验 | 等待外部条件 | V11 首个权重初始化训练完成，名称与别名联合验证正确率为 99.11%；用户将清理 `data.json` 的不合常理组合。待模板修改后重建数据版本，再从 V11 检查点进行首个对照训练。 |
| 16 | 校准与接受策略 | 等待外部条件 | V10 无可校准候选。待下一数据版本改善名称与别名跨模板组泛化后，仅使用新版本 `validation.jsonl` 校准。 |
| 17 | 独立评测与发布判定 | 等待外部条件 | V10 未运行冻结模型评测，但为定位失败已检查冻结分区模板结构，不能再用作独立验收。新候选须重新生成冻结分区，并满足名称与别名联合≥95%、季数≥97%、集数≥97%、核心字段整条≥95%，方可读取一次新的冻结测试。 |
