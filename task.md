...
- 模板支持 `%random_range(起点-终点)` 生成不补零的闭区间随机整数，`%random_str($字符串来源, 最大连续字符数)` 从已渲染来源字段随机取得连续片段；`title_alias_begin` 明确主标题与首个别名之间的边界，`$title_alias[1]` 选择第一个非空别名，`$random_month` 使用模板组内月份词表生成未标注格式文本；设置 `exclude_title_season_expr: true` 的模板只选择标题自身没有季数表达的条目，并由同模板组 `season_expr` 词表提供季数实体。
...
- 模型实际验证页面用于输入真实标题并观察模型输出，重点核对名称、别名、季数和集数；不写入训练数据，也不是当前训练数据的前置条件。
...
│   └── serving/              # 可选 HTTP 服务
├── tools/
│   ├── model_verify_app.py    # 中文模型实际验证页面
...
| 维度 | 指标 | 计算边界 |
| --- | --- | --- |
| 字段抽取 | 严格片段级 Precision / Recall / micro-F1、macro-F1 | 用于诊断全部字段，字段类型和边界同时正确才算命中，排除未标注区域 |
| 核心字段规范化 | 名称与别名联合、季数、集数完全正确率 | 名称与别名可被模型合并为一个连续实体；季数和集数按适用样本分别统计 |
| 核心完整解析 | 核心字段整条完全正确率 | 只比较名称/别名、季数和集数；发布组与技术字段不计入发布门槛 |
...
- 名称与别名联合完全正确率目标≥95%。标题和别名的标签类型可互换，也允许合并成一个连续名称实体；但整体起止边界必须正确。
- 季数与集数分别在适用样本上的完全正确率≥97%；集数表达、范围和声明总集数均属于集数核对范围。必须注明样本数量与覆盖率。
- 核心字段整条完全正确率目标≥95%，仅比较名称与别名联合、季数和集数；不因发布组、片源、分辨率、编码、字幕或其他技术字段失败而阻断发布。
- 发布组和其他非核心字段仍须在冻结报告中按字段列出，用于发现回归，但仅作观测指标。关键困难类型按名称/别名、季数和集数分层报告，并按作品组估计95%置信区间。
